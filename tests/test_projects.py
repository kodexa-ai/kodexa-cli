...
def test_export_project(cli_runner, mock_kodexa_client):
...
def test_export_project_with_profile(cli_runner, mock_kodexa_client, mock_kodexa_platform):
...
def test_import_project(cli_runner, mock_kodexa_client):
...
def test_bootstrap(cli_runner, mock_kodexa_client):
...
def test_bootstrap_with_profile(cli_runner, mock_kodexa_client, mock_kodexa_platform):
...
def test_send_event(cli_runner, mock_kodexa_client):
...
def test_send_event_with_profile(cli_runner, mock_kodexa_client, mock_kodexa_platform):
